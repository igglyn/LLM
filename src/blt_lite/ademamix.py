...
class AdEMAMix(Optimizer):
    ...
    def reset_slow_ema(self):
        """Reset the slow EMA accumulators to zero.

        Call periodically to prevent stale long-range gradient history
        from destabilizing training when loss enters a low-gradient regime.
        The fast EMA, second moment, and step counter are preserved.
        """
        for group in self.param_groups:
            for p in group['params']:
                state = self.state[p]
                if 'exp_avg_slow' in state:
                    state['exp_avg_slow'].zero_()
