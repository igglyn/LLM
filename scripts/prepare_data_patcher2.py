...
def main():
    parser = argparse.ArgumentParser(description="Prepare explicit stage-2 (patcher2) training token artifacts.")
    parser.add_argument("--config", required=True)
    args = parser.parse_args()

    cfg = load_config(args.config)
    data_cfg = cfg["data"]
    source_dir = Path(data_cfg.get("processed_dir_patcher", data_cfg["processed_dir"]))
    out_dir = Path(data_cfg.get("processed_dir_patcher2", "data/processed/patcher2"))

    summary = _copy_stage(source_dir, out_dir)
    summary["seq_len_tokens"] = int(cfg.get("patcher2_train", {}).get("seq_len_tokens", 0))
