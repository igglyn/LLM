...
def _encode_stream(
    ...
    with torch.no_grad():
        for start in range(0, len(tokens), chunk):
            ...
            out_hidden = h2[:, take_from:, :]
            if verify_token_identity:
                target = x[:, take_from:]
                stage1_decoded = _decode_to_tokens(h1[:, take_from:, :], emb_weight)
                stage1_mismatch += int((stage1_decoded != target).sum().item())
                if p2 is not None:
                    stage2_decoded = _decode_to_tokens(out_hidden, emb_weight)
                    stage2_mismatch += int((stage2_decoded != target).sum().item())
    ...
    return {
        ...
        "stage1_mismatch_tokens": stage1_mismatch,
        "stage2_mismatch_tokens": stage2_mismatch,
    }
...
def main():
    ...
    train_verify = _encode_stream(
        ...
        seq_len,
        device,
        out_dir / "train_stage2_hidden.npy",
        d_model,
    ...
    if args.verify_token_identity:
        ...
        stage1_mismatch = int(train_verify["stage1_mismatch_tokens"] + val_verify["stage1_mismatch_tokens"])
        stage2_mismatch = int(train_verify["stage2_mismatch_tokens"] + val_verify["stage2_mismatch_tokens"])
        stage1_mismatch_rate = (float(stage1_mismatch) / float(total_checked)) if total_checked > 0 else 0.0
        stage2_mismatch_rate = (float(stage2_mismatch) / float(total_checked)) if total_checked > 0 else 0.0
        fail_rate = float(args.verify_token_identity_fail_rate)
        if stage1_mismatch_rate > fail_rate:
            raise RuntimeError(
                "Token identity check failed for patcher reconstruction: "
                f"{stage1_mismatch}/{total_checked} mismatched (rate={stage1_mismatch_rate:.12g}, threshold={fail_rate:.12g})"
            )
        if patcher2_enabled and stage2_mismatch_rate > fail_rate:
            raise RuntimeError(
                "Token identity check failed for patcher2 reconstruction: "
                f"{stage2_mismatch}/{total_checked} mismatched (rate={stage2_mismatch_rate:.12g}, threshold={fail_rate:.12g})"
            )
    else:
        stage1_mismatch_rate = None
        stage2_mismatch_rate = None
        fail_rate = float(args.verify_token_identity_fail_rate)
    ...
    summary = {
        ...
        "token_identity_check": {
            ...
            "stage1_total_mismatch_rate": stage1_mismatch_rate,
            "stage2_total_mismatch_rate": stage2_mismatch_rate,
            "train": train_verify,
            "val": val_verify,
        },
    }
