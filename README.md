...
- Patcher architecture and training knobs are configurable under `patcher` and `patcher_train` in the YAML config. LM training uses AdEMAMix, while patcher pretraining uses AdamW; patcher pretraining can reduce LR automatically once `patcher_train.lr_reduce_threshold` is reached.
